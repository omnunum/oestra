...
import { cloneDeep, min, max, map, each, get, sum, groupBy} from 'lodash';
...
export class Portfolio {
  constructor(initialCash = 0) {
      ...
      this.filings = reactive({ [DateTime.utc().year - 1]: reactive({}) });
  ...
  getTaxInfo(year, region, status, capitalGains = null) {
      ...
      if (region == 'federal' & capitalGains) {
  ...
  calculateAmtTaxes(year) {
      ...
      var exercised = map(lifecycles, (l) => { if (l.stock != null) {return l}});
      var exerciseSpread = sum(map(exercised, (l) => { 
